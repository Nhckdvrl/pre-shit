...
def run(repo, revision, stim, device, batch_size):
    ...
    for i, it in enumerate(stim):
        cands = [("current", it["current"])]
        cands += [("interferer", v) for v in it["interferers"]]
        cands += [("unmentioned", v) for v in it["unmentioned"]]
        for role, cand in cands:
            jobs.append((it["prompt"], cand))
            keys.append((i, it["trial"], it["category"], it["condition"],
                         it["n_states"], role, cand))
...
def main():
    ...
    for seed in [int(x) for x in args.seeds.split(",")]:
        ...
        for step in [int(x) for x in args.steps.split(",")]:
            ...
            df = pd.DataFrame(keys, columns=["idx", "trial", "category", "condition",
                                              "n_states", "role", "candidate"])
            df["logp"] = vals
